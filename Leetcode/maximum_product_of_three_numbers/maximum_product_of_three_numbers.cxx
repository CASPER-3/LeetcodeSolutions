...
public:
    ...
    int maximumProduct(vector<int> &nums)
    {
        ...
        return nums[0] > 0 || nums.size() == 3 ? nums[nums.size()-1] * nums[nums.size()-2] * nums[nums.size()-3] : nums[0] * nums[1] * nums[nums.size() - 1] > nums[nums.size() - 1] * nums[nums.size() - 2] * nums[nums.size() - 3] ? nums[0] * nums[1] * nums[nums.size() - 1]
